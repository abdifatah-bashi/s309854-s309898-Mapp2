...
public class BestillingListeAdapter extends ArrayAdapter<Bestilling> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        Bestilling best = databaseHelper.getBestilling(id_best);

                String tid = best.getDato() + " " + best.getKlokkeslett(); //getItem(position).getDato() + " " + getItem(position).getKlokkeslett();
                Restaurant restaurant = databaseHelper.getRestaurant(best.getRestaurant_id());
                String restaurantNavn = restaurant.getNavn();

                List<Venn> venner = databaseHelper.hentVenner_Bestilling(best);
                ...
                // final Bestilling bestilling = new Bestilling(getItem(position).getDato(), getItem(position).getKlokkeslett(),
                //       getItem(position).getRestaurant_id());
